function Cart({ cart, onRemove, onCheckout, onBack }) {
  ...
  return (
    <div
      id="cart"
      className="mx-auto max-w-5xl rounded-2xl border border-slate-200 bg-white p-6 shadow-card md:p-8"
    >
      ...
      <div className="mt-5 space-y-4">
        {cart.length === 0 ? (
          ...
          cart.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between gap-4 rounded-xl bg-[#F7F7FB] px-4 py-4"
            >
      ...
      {cart.length > 0 && (
        <>
          ...
          <button
            onClick={onCheckout}
            className="mt-4 w-full rounded-full bg-gradient-to-r from-[#6D28F9] to-[#B620F2] px-6 py-3 text-sm font-semibold text-white"
          >
            Proceed To Checkout
          </button>
        </>
